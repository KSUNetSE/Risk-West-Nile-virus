# -*- coding: utf-8 -*-
"""
Created on Wed Apr 10 19:59:15 2024
...
values=np.cumsum(list(np.loadtxt("./orangecases2008.txt")))
dama = np.loadtxt('orange-mintem-2008.txt')
...
#plt.scatter(t, values, label='Data')
...
def datagen(C,P):
    ...
    x0=(3000000,0,0,0,1000,1000,1000,1000,10,100,100,100,100,100,100)
    x=odeint(BMPPT,x0,t)
    R=x[0:365,3]
    I=x[0:365,2]
    #plt.plot(R)
...
#--------------------------------MCMC
def ll_function(data,C,P):#mean = data which was produced by the quation and data = the values of R
    testdata=datagen(C,P)[0]
    std_v=10
    #loglikelihood = (np.prod(np.exp((-0.5/std_v**2) * ((data -testdata)**2))))
    #loglikelihood = (1/sum((data -testdata)**2))
    error=np.mean((data -testdata)**2)
    loglikelihood=1/error
    return loglikelihood,error
#print(ll_function(exp_data,C))

#param0=np.loadtxt('C2008_29000.txt')
y0=[0 for i in range(0,364)]
P0=[1 for i in range(0,364)]
# plt.plot(param0)
# plt.show()
param0=[2500000000  for i in range(0,365)]
...
for i in range(0,run_num):
    ...
    if i % 1== 0:
        ...
        # filename = f'C2008_{i}.txt'
        # np.savetxt(filename, param0, fmt='%d')
        plt.plot(t, logistic_function(tt, *popt), label='Fit', color='red')
        plt.plot(datagen(param0,P0)[0])
        plt.xlabel('Time')
        plt.ylabel('Values')
        plt.title(i)
        plt.show()
    ...
    std=25000000
    s=10
    y=np.random.normal(0,s,364)
    #P=norm.cdf(y, loc=0, scale=s)
    P=[1 for i in range(0,364)]
    while True:
        paramtest=np.random.normal(param0,std)
        z=ll_function(exp_data,paramtest,P)
        #print(z[0])
        if z[0]>0:
            break
    
    all_value.append(sum(paramtest))
    z0=ll_function(exp_data,param0,P0)
    u=z[0]/z0[0]
    print(u)
    E.append(z[1])
    T.append(paramtest)
