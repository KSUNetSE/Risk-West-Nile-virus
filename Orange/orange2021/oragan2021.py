...
values=np.cumsum(list(np.loadtxt("./oregancases2007.txt")))
dama = np.loadtxt('orange-mintem-2007.txt')
...
def ll_function(data,C):#mean = data which was produced by the quation and data = the values of R
    ...
    #print((testdata))
    std_v=1
    #loglikelihood1 = np.prod((np.sqrt(2*np.pi*std_dev**2))*norm.pdf(data, testdata, std_dev))
    #loglikelihood = (np.prod(np.exp((-0.5/std_v**2) * ((data -testdata)**2))))
    loglikelihood = (1/sum((data -testdata)**2))

    #pp=(np.prod(np.exp((-0.5/std_dev**2) * ((data -10000000*np.ones(364))**2))))
    #print("pppp",pp)
    error=np.mean((data -testdata)**2)
    return loglikelihood,error
#print(ll_function(exp_data,C))

param0=np.loadtxt('C2007orange.txt')
# plt.plot(param0)
# plt.show()
#param0=[699000000000  for i in range(0,365)]
...
for i in range(0,run_num):
    if i % 500 == 0:
        ...
        filename = f'C2007_{i}.txt'
        np.savetxt(filename, param0, fmt='%d')
        plt.plot(t, logistic_function(tt, *popt), label='Fit', color='red')
        plt.plot(datagen(param0)[0])
        plt.xlabel('Time')
        plt.ylabel('Values')
        plt.title(i)
        plt.show()
    ...
    E.append(z[1])
    T.append(paramtest)
    #print(i,u,z[2],sum(exp_data),(z[1]))
