...
values=np.cumsum(list(np.loadtxt("./orangecases2011.txt")))
dama = np.loadtxt('orange-mintem-2011.txt')
...
param0=np.loadtxt('C2009_14100.txt')
ref=[18900000000  for i in range(0,365)]
#param0=[18900000000  for i in range(0,365)]
...
E=[]
T=[]
for i in range(14101,run_num):
    if ll_function(exp_data,param0)[1]<0.01:
        break
    else:

        if i % 1 == 0:
            # Format the filename to include the iteration number
            filename = f'C2011_{i}.txt'
