# -*- coding: utf-8 -*-
"""
Created on Tue Apr  2 15:07:30 2024
...
values=np.cumsum(list(np.loadtxt("./orengecases2006.txt")))
...
#dama = np.loadtxt('orange-TS-tem-2006.txt')
dama = np.loadtxt('min-tem.txt')
...
def datagen(C):
    ...
    def BMPPT(x,time):
        ...
        dacuMdt=Coematrix[5,time]*x[4]*max(0,(1-(x[5]/(C[0]))))-Coematrix[0,time]*Coematrix[1,time]*x[5]
...
for i in range(0,run_num):
    if divmod(i,100)[1]==0:
        #np.savetxt('C2006orange.txt', param0, fmt='%d')
        plt.plot(t, logistic_function(tt, *popt), label='Fit', color='red')
    ...
    if  u>1:
        param0=paramtest
        print(i,u,z[0],"error",z[1],"param0 ",param0)
        # plt.plot(t, logistic_function(tt, *popt), label='Fit', color='red')
        # plt.plot(datagen(param0)[0])
        # plt.xlabel('Time')
        # plt.ylabel('Values')
