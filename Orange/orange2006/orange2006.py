...
values=np.cumsum(list(np.loadtxt("./orangecases2006.txt")))
dama = np.loadtxt('orange-mintem-2006.txt')
...
def ll_function(data,C):#mean = data which was produced by the quation and data = the values of R
    ...
    error=sum(np.abs(data -testdata))/365
    return loglikelihood,error,maxtes


param0=np.loadtxt('C2006_17900.txt')
ref=[10100000000  for i in range(0,365)]
#ref=np.loadtxt('C2006_100.txt')
#param0=[10100000000  for i in range(0,365)]
...
T=[]
std=150000
for i in range(17901,run_num):
    if i % 1== 0:
        filename = f'C2006_{i}.txt'
