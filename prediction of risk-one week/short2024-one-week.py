...
years = [year for year in range(2006, 2024) if year != 2010]#list(range(2006, 2024))  # Years from 2006 to 2023 (excluding 2024)
print(years)
...
# def forecast_ar(data, steps=7,lags=1):
#     """
...
for i in range(starting_time,365,prediction_lead):
    ...
    predictions = forecast_ar(inuse, steps=7, lags=7)
    ...
    predictionsH = forecast_ar(inuseH, steps=7, lags=7)
    ...
    predictionsP = forecast_ar(inuseP, steps=7, lags=7)
    ...
    print("day",d,"prediction",find_best_x_for_point(m, w, interpolators_dict)[0])
    cases.append(find_best_x_for_point(m, w, interpolators_dict)[0])
...
def calculate_log_likelihood(A, B):
    ...
    total_log_likelihood = np.round(sum(log_likelihoods) / len(log_likelihoods)  , decimals=3)
...
A=[0] * (52-len(A2)) + A2
visualize_risk(cases, MM, 2024, A, B)
