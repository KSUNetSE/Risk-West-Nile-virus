...
years = list(range(2006, 2023))  # Years from 2006 to 2022 (excluding 2023)
...
with open("Curve-2023.txt", "w") as file:
    for item in Curve:
...
def kernel_frequency_surface(coordinates, bw=0.3):
    ...
    np.savez("2d_rate_2023_short.npz", X=grid_x, Y=grid_y, Z=grid_z)
    ...
    parameter = []
    parameter = np.loadtxt("Curve-2023.txt")
...
def plot_curve_with_fixed_shadow_and_uniform_pdf(coordinates, r=100, grid_res=100):
    ...
    # --- Save to NPZ ---
    np.savez("2d_prior_test_2023_short.npz",
              area=area_of_region,
              X=X,
              Y=Y,
              Z=Z)
    #print("Saved '2d_prior_test_2023.npz_short' with keys: [area, X, Y, Z].")
...
#for year in years:
year=2023
T=list(get_temperature_for_year(file_path, 2023)[1])
H=list(get_temperature_for_year(file_path, 2023)[2])
P=list(get_temperature_for_year(file_path, 2023)[3])
...
for i in range(starting_time,365,prediction_lead):
    ...
    for j in range(0, len(mosquito_profiles)):
        #print(j)
        Curve2023.append((mosquito_profiles[j], BR_dic[j]))
    ...
    def posterior_array(M_array, W_array, x_value, prior_file="2d_prior_test_2023_short.npz", rate_file="2d_rate_2023_short.npz"):
        """
    ...
    for i in range(tt, 32):
        pdf_file = "2d_prior_test_2023_short.npz"
        ...
        Z = posterior_array(X, Y, x_value,
                            prior_file="2d_prior_test_2023_short.npz",
                            rate_file="2d_rate_2023_short.npz")
        ...
        np.save(f'posterior_x_{x_value}_2023_short.npy', posterior_array_4cols)
        ...
        parameters = []
        parameters = np.loadtxt("Curve-2023.txt")
    ...
    def preload_posteriors(x_values=range(tt,32)):
        ...
        for x_val in x_values:
            filename = f"posterior_x_{x_val}_2023_short.npy"
...
print(len(A))
visualize_risk(cases, MM, 2023, A, B)
