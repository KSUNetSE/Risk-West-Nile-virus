...
# # Example usage
file_path = 'Riversideinfo.csv'  # Replace with your actual file path
...
# H2022 = list(np.loadtxt("./2022forecasted_Hum.txt"))
# P2022 = list(np.loadtxt("./2022forecasted_Pre.txt"))

T2023 = list(np.loadtxt("./2023forecasted_Tem.txt"))
H2023 = list(np.loadtxt("./2023forecasted_Hum.txt"))
P2023 = list(np.loadtxt("./2023forecasted_Pre.txt"))

# window_size=10
# Cf2006 = moving_average_numpy_full(Cf2006, window_size)
# Cf2007 = moving_average_numpy_full(Cf2007, window_size)
# Cf2008 = moving_average_numpy_full(Cf2008, window_size)
# Cf2009 = moving_average_numpy_full(Cf2009, window_size)
# #C2010 = moving_average_numpy_full(C2010, window_size)
# Cf2011 = moving_average_numpy_full(Cf2011, window_size)
# Cf2012 = moving_average_numpy_full(Cf2012, window_size)
# Cf2013 = moving_average_numpy_full(Cf2013, window_size)
# Cf2014 = moving_average_numpy_full(Cf2014, window_size)
# Cf2015 = moving_average_numpy_full(Cf2015, window_size)
# Cf2016 = moving_average_numpy_full(Cf2016, window_size)
# Cf2017 = moving_average_numpy_full(Cf2017, window_size)
# Cf2018 = moving_average_numpy_full(Cf2018, window_size)
# Cf2019 = moving_average_numpy_full(Cf2019, window_size)
# Cf2020 = moving_average_numpy_full(Cf2020, window_size)
...
maxpre=[]
for year in years:
    m=max(get_temperature_for_year(file_path, year)[0])
    maxpre.append(m)
print(maxpre)
print(max(maxpre))
...
data_dict_Temp={}
data_dict_Humid={}
prerange=[i/100 for i in range(0,100*int(np.ceil(max(maxpre))))]
for h in prerange:
    # print("h",h)
    diclist=[]
    diclisttem=[]
...
plt.plot(Coematrix[8,:],'g-')



data=Coematrix[8,:]
d=1
Q1 = np.percentile(data, d)
Q3 = np.percentile(data, 100-d)
...
# Substitute outliers with the average of the five values around them
for idx in outlier_indices:
    start_idx = max(0, idx - 2)
    end_idx = min(len(data), idx + 3)
    surrounding_values = np.delete(data[start_idx:end_idx], idx - start_idx)
    if len(surrounding_values) > 0:
        data[idx] = np.mean(surrounding_values)

# Print the results
print("Original Data with Outliers Removed:")
print(data)
print("\nIndices of Outliers:")
print(outlier_indices)
window_size=8
Coematrix[8,:]=moving_average_numpy_full(data, window_size)
Coematrix[8,:]=data
plt.plot(Coematrix[8,:],'b-')
plt.show()
...
np.savetxt(f'C_esti_{Year}.txt', Coematrix[8,:])
