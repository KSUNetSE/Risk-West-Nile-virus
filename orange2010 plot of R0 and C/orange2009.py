# -*- coding: utf-8 -*-
"""
Created on Sat Apr  6 13:05:59 2024
...
values=np.cumsum(list(np.loadtxt("./orangecases2009.txt")))
dama = np.loadtxt('orange-mintem-2009.txt')
...
# Convert R0 and C into pandas Series for easy rolling computation
R0_series = pd.Series(R0)
C_series = pd.Series(C)




# Set window size (adjust based on data smoothness requirement)
window_size = 10  # Example: 7-day moving average

# Compute moving averages
R0_moving_avg = moving_average(R0_series, window_size)
C_moving_avg = moving_average(C_series, window_size)
...
# Collect temperature and R0 values
tem = []
RR0 = []
for i in range(0, 364):
    tem.append(tim2tem(i))
    RR0.append(R0[i])

paired = list(zip(tem, RR0))

# Sort the pairs based on the temperature (the first element of each pair)
sorted_pairs = sorted(paired, key=lambda x: x[0])

# Unzip the sorted pairs back into two lists
sorted_tem, sorted_RR0 = zip(*sorted_pairs)

# Convert the result to lists (if needed)
sorted_tem = list(sorted_tem)
sorted_RR0 = list(sorted_RR0)

 
MRR0 = moving_average(sorted_RR0, 10)  



# Plot moving average of R0
plt.figure(figsize=(8, 5))
plt.plot(R0_series, alpha=0.3, color='blue')  # Plot original with transparency
plt.plot(R0_moving_avg, color='blue')
plt.xlabel("Time", fontsize=14)
plt.ylabel("$R_0$", fontsize=14)
plt.title('Daily Values of $R_0$, Orange County, California-2009', fontsize=15)
plt.legend()
plt.grid(False)
plt.savefig("R0_moving_average.pdf", format="pdf")
plt.show()


# Plot moving average of C
plt.figure(figsize=(8, 5))
plt.plot(C_series, alpha=0.3, color='blue')  # Plot original with transparency
plt.plot(C_moving_avg, color='blue')
plt.xlabel("Time", fontsize=14)
plt.ylabel("C", fontsize=14)
plt.title("Carrying Capacity, Orange County, California-2009", fontsize=15)
plt.legend()
plt.grid(False)
plt.savefig("Carrying_Capacity.pdf", format="pdf")
plt.show()


# Plot sorted R0 vs Temperature
plt.figure(figsize=(8, 5))
plt.plot(sorted_tem, sorted_RR0, '--', alpha=0.3, color='blue')  # Original data (dashed, transparent)
plt.plot(sorted_tem, MRR0, '--b')  # 10-point moving average
plt.xlabel('Temperature', fontsize=14)
plt.ylabel('$R_{0}$', fontsize=14)
plt.title('$R_0$, Orange County, California-2009', fontsize=15)
plt.legend()
plt.grid(False)
plt.savefig("R0_vs_Temperature.pdf", format="pdf")
plt.show()
