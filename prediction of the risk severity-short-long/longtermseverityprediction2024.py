...
years = list(range(2006, 2024))  # Years from 2006 to 2023 (excluding 2021)
...
Time=list(np.loadtxt('./orangecases2024.txt'))    # vector of cases to find out atarting point
plt.plot(Time)
plt.show()
...
T_train = T[:-(1 * days_in_year)] # Remove 2023-2024 data
H_train = H[:-(1 * days_in_year)]
P_train = P[:-(1 * days_in_year)]
...
T_predicted = T_forecast.tolist()
H_predicted = H_forecast.tolist()
P_predicted = P_forecast.tolist()

# Combine the last few actual days from early 2023 with the forecasted data
days_from_2022_used =0  # Example: Using 10 real days of 2023
T =  T_predicted
H = H_predicted
P = P_predicted
...
# Separate 2024 data (last 365 days of the dataset)
# T = T[-days_in_year:]  # Temperature data for 2024
# H = H[-days_in_year:]  # Humidity data for 2024
# P = P[-days_in_year:]  # Precipitation data for 2024



plt.plot(T)#_predicted)
plt.show()
...
basicR0=R0
np.savetxt("./MMosquito_Profile_2024_predictedoneyear.txt", Mos, fmt="%.6f")
mosquito_profiles =M



T=np.array(T)#_predicted)#_predicted )    
H=np.array(H)#_predicted)#_predicted )
P=np.array(P)#_predicted)#_predicted)
...
if year == 2024:
    ...
    for i in range(0, 364):
        #print(i)
        Curve.append((M[i], BR_dic[i]))
    #print(Curve)
    
    # Save to a text file in two columns format
    with open("Curve_predicted_lomgter_2024.txt", "w") as file:
        for item in Curve:
...
def kernel_frequency_surface(coordinates, bw=0.3):
    ...
    np.savez("2d_rate_2024_predicted.npz", X=grid_x, Y=grid_y, Z=grid_z)
    print("Scaled 2D rate saved as '2d_rate_2024_longtermprediction.npz'")
    ...
    parameter = []
    parameter = np.loadtxt("Curve_predicted_lomgter_2024.txt")
    # parameter = np.loadtxt("Curve2024")#_predicted.txt")
...
def plot_curve_with_fixed_shadow_and_uniform_pdf(coordinates, r=100, grid_res=100):
    ...
    plt.legend()
    plt.savefig('prior_uniform_2024_predicted.pdf', format='pdf', bbox_inches='tight', dpi=1000)
    plt.show()
    ...
    # --- Save to NPZ ---
    np.savez("2d_prior_test_2024_predicted.npz",
              area=area_of_region,
              X=X,
              Y=Y,
              Z=Z)
    print("Saved '2d_prior_test_2024_predicted.npz' with keys: [area, X, Y, Z].")
...
def posterior_array(M_array, W_array, x_value, prior_file="2d_prior_test_2024_predicted.npz", rate_file="2d_rate_2024_predicted.npz"):
    """
...
for i in range(tt, 32):
    pdf_file = "2d_prior_test_2024_predicted.npz"
    ...
    Z = posterior_array(X, Y, x_value,
                        prior_file="2d_prior_test_2024_predicted.npz",
                        rate_file="2d_rate_2024_predicted.npz")
    ...
    np.save(f'posterior_x_{x_value}_2024_predicted.npy', posterior_array_4cols)
...
def preload_posteriors(x_values=range(tt,32)):
    ...
    for x_val in x_values:
        filename = f"posterior_x_{x_val}_2024_predicted.npy"
...
file_path = 'Curve_predicted_lomgter_2024.txt'
...
def visualize_risk(predicted_cases, mosquito_profile, Casetime, year, starting_point):
    ...
    #             bbox_to_anchor=(0.3, 0.85), fontsize=20, 
    #             title_fontsize=20)
...
MM=list(np.loadtxt("./MMosquito_Profile_2024_predictedoneyear.txt"))
...
print("x",calculate_log_likelihood(A, B))
...
print("y",calculate_log_likelihood(AA, B))
...
def plot_model_comparison(A, AA, B, year, x, y):
    ...
    plt.plot(weeks, A[starting:52], marker='o', linestyle='-', label=f'Probabilistic Model (Score = {np.round(x, decimals=2)})', linewidth=2)
    ...
    #plt.title(f'Model Comparison-Long-term- Orange County, Califortnia - {year}', fontsize=15)
