...
years = list(range(2006, 2023))  # Years from 2006 to 2023 (excluding 2021)
...
Time=list(np.loadtxt('./orangecases2023.txt'))    # vector of cases to find out atarting point
plt.plot(Time)
plt.show()
...
T_train = T[:-(2 * days_in_year)] # Remove 2023-2024 data
H_train = H[:-(2 * days_in_year)]
P_train = P[:-(2 * days_in_year)]
...
# T = T[-(2 * days_in_year):-days_in_year]  # Temperature data for 2023
# H= H[-(2 * days_in_year):-days_in_year]  # Humidity data for 2023
# P= P[-(2 * days_in_year):-days_in_year]  # Precipitation data for 2023
...
basicR0=R0
np.savetxt("./MMosquito_Profile_2023_predictedoneyear.txt", Mos, fmt="%.6f")
mosquito_profiles =M
...
if year == 2023:
    ...
    for i in range(0, 364):
        print(i)
        Curve2023_predicted.append((mosquito_profiles[i], BR_dic[i]))
    print(Curve2023_predicted)
    
    # Save to a text file in two columns format
    with open("Curve2023_predicted.txt", "w") as file:
        for item in Curve2023_predicted:
...
def kernel_frequency_surface(coordinates, bw=0.3):
    ...
    np.savez("2d_rate_2023_predicted.npz", X=grid_x, Y=grid_y, Z=grid_z)
    print("Scaled 2D rate saved as '2d_rate_2023_longtermprediction.npz'")
    ...
    parameter = []
    parameter = np.loadtxt("Curve2023_predicted.txt")
...
def plot_curve_with_fixed_shadow_and_uniform_pdf(coordinates, r=100, grid_res=100):
    ...
    plt.legend()
    plt.savefig('prior_uniform_2023_predicted.pdf', format='pdf', bbox_inches='tight', dpi=1000)
    plt.show()
    ...
    # --- Save to NPZ ---
    np.savez("2d_prior_test_2023_predicted.npz",
              area=area_of_region,
              X=X,
              Y=Y,
              Z=Z)
    print("Saved '2d_prior_test_2023_predicted.npz' with keys: [area, X, Y, Z].")
...
def posterior_array(M_array, W_array, x_value, prior_file="2d_prior_test_2023_predicted.npz", rate_file="2d_rate_2023_predicted.npz"):
    """
...
for i in range(tt, 32):
    pdf_file = "2d_prior_test_2023_predicted.npz"
    ...
    Z = posterior_array(X, Y, x_value,
                        prior_file="2d_prior_test_2023_predicted.npz",
                        rate_file="2d_rate_2023_predicted.npz")
    ...
    np.save(f'posterior_x_{x_value}_2023_predicted.npy', posterior_array_4cols)
...
def preload_posteriors(x_values=range(tt,32)):
    ...
    for x_val in x_values:
        filename = f"posterior_x_{x_val}_2023_predicted.npy"
...
file_path = 'Curve2023_predicted.txt'
...
MM=list(np.loadtxt("./MMosquito_Profile_2023_predictedoneyear.txt"))
