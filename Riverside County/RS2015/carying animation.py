# -*- coding: utf-8 -*-
"""
Created on Wed Jun  5 10:33:21 2024
...
for i in range(100, 30001, 500):
    ...
    def BMPPT(x,time):
        ...
        dacuMdt=Coematrix[5,time]*x[4]*max(0,(1-(x[5]/Cr)))-Coematrix[0,time]*Coematrix[1,time]*x[5]
    ...
    MS=x[0:r*365,6]
    R=x[0:r*365,3]
    plt.plot(R[(d)*365:dd*365], 'b--')
    C = np.loadtxt(f'C_RS_2015_{i}.txt')
    #print(C)
    @jit
    def BMPPT(x,time):
        time=int(divmod(time, 365)[1])
    ...
    MS=x[0:r*365,6]
    R=x[0:r*365,3]
    plt.plot(R[(d)*365:dd*365], 'g--')
    plt.plot(logistic_function(tt, *popt), 'r-')
    plt.show()
