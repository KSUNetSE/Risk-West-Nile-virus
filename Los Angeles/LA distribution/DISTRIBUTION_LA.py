...
for year in years:
    ...
    Casetime=list(np.loadtxt(f"./LA{year}.txt"))
    ...
    x0=(10014009,0,0,0,10,10,10,10,10,100,100,100,100,100,100)
    x=odeint(BMPPT,x0,t)
    muB=(0.15/365)
    incB=1/3
    rC=1/6
    ...
    axes[0].set_title('Losangeles County, California - ' + str(year))
    axes[0].legend()
    axes[0].grid(True)
    ...
    axes[1].set_title('Lsangeles County, California - ' + str(year))
    axes[1].legend()
    axes[1].grid(True)
    
    plt.tight_layout()
    #plt.savefig(f'Profile_and_R0_Butte_county_{year}.pdf', format='pdf', dpi=1000)
    plt.savefig(f'Profile_LA_county{year}.jpg', format='jpg', dpi=1000)
    plt.show()
...
n = np.linspace(0, 4*max(N_array), 100)
br = np.linspace(0, 4*max(BR_array), 100)
...
ax.set_xlabel('Mosquitoe population size N')
ax.set_ylabel('Basic reproductive number')
ax.set_title('Joint PDF with Contour Levels')
plt.grid(True)
plt.savefig('jointpdfLA.jpg', format='jpg', dpi=1000)
plt.show()
...
# plt.hist(N, bins=15, density=True, color='blue', edgecolor='black', alpha=0.6)
# plt.plot(x, pdf_fitted, 'r-', label=f'Gamma PDF\nshape={a:.2f}, scale={scale:.2f}')
# plt.title('Histogram of Data with Fitted Gamma PDF')
# plt.xlabel('Value')
# plt.ylabel('Density')
# plt.legend()
...
# plt.figure(figsize=(10, 6))
# plt.hist(N1, bins=15, density=True, color='blue', edgecolor='black', alpha=0.6)
...
for year in years:
    if year>=1981 and year<=2000:
        Casetime=list(np.loadtxt(f"./LA{2006}.txt"))
        C=list(np.loadtxt(f"./C{year}.txt"))
        ...
        if year==2021 or year==2023:
            Casetime=list(np.loadtxt("./LA2021.txt"))
        elif year == 2022:
            Casetime=list(np.loadtxt("./LA2022.txt"))
        dama = list(get_temperature_for_year(file_path, year)[0])
...
#------------------------------------------------------------------------------
    ...
    #         red=red+1
    # yellyear.append(yell/365)
    # redyear.append(red/365)
    # oranyear.append(oran/365)
    # greenyear.append(green/365)
    # # redtogreen.append(red/green)
    # risktoyear.append(1-(green/365))
    # relativeredtorisk.append(red/(red+oran+yell))
    for i in range(0,364):
        r=0 
    ...
    if year<2010:
        ...
        plt.title('Estimated Risk-Los Angeles County - '+str(year))
        plt.legend()
        ...
        plt.title('Predicted Risk-Los Angeles County - '+str(year))
        plt.legend()
        plt.savefig(f'risk{year}.jpg', format='jpg', dpi=1000)
        plt.show()
        #--------------------------------------------------------------------
