...
values=np.cumsum(list(np.loadtxt("./LA2020.txt")))
dama = np.loadtxt('LA-tem-2020.txt')
...
param0=np.loadtxt('C_LA_2020_6300.txt')
ref=[30000000000  for i in range(0,365)]
#param0=[30000000000  for i in range(0,365)]
...
for i in range(6301,run_num):
    if i % 100== 0:
        filename = f'C_LA_2020_{i}.txt'
        ...
        plt.ylabel('Values')
        plt.title(i)
        plt.title(f'{i}: {ll_function(exp_data,param0)[1]}  '+str(2020))
        plt.legend()
        plt.show()
        plt.plot(param0)
        plt.show()
